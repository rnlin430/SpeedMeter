...
public class SpeedMeter extends JavaPlugin {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (command.getName().equalsIgnoreCase("speedmeter")) {
			...
			case 2:
				if(args[0].equalsIgnoreCase("r")) {
					...
					sender.sendMessage(ChatColor.GRAY + "playerSpeedThresholdValue:" + config.getDouble("playerSpeedThresholdValue"));
					return true;
					...
					sender.sendMessage(ChatColor.GRAY + "Other_Vehicle_Speed_Threshold:" + config.getDouble("Other_Vehicle_Speed_Threshold"));
				}else if(args[0].equalsIgnoreCase("pst")) {
					FileConfiguration config = getConfig();
					config.set("Vehicle_Speed_Threshold", args[1]);
					saveConfig();
					PlayerMoveListener.vehicleSpeedThresholdValue = Double.parseDouble(args[1]);
					sender.sendMessage(ChatColor.GRAY + "Vehicle_Speed_Threshold:" + config.getDouble("Vehicle_Speed_Threshold"));
					return true;
				}else {
					return false;
				}
		 ...
		 */
						...
						if((SpeedMeter.manager.discreteMeterOnOff.get(player.getName()))) {
							if(allMeterOnOffList.get(player.getName()) || !allMeterOnOffList.containsKey(player.getName())) {
								...
								sender.sendMessage(ChatColor.GOLD + "" + ChatColor.BOLD +
										"Tip: " + ChatColor.GRAY + "" + ChatColor.BOLD + "/allmeter off - 乗り物に乗っているときにだけ速度を表示します。");
							}
							else if(!allMeterOnOffList.get(player.getName()) || !allMeterOnOffList.containsKey(player.getName())) {
								sender.sendMessage(ChatColor.AQUA +  "" + ChatColor.BOLD + "info: 乗り物に乗ってるときにだけ速度を表示します。");
								sender.sendMessage(ChatColor.GOLD + "" + ChatColor.BOLD +
										"Tip: " + ChatColor.GRAY + "" + ChatColor.BOLD + "/allmeter on - 常に速度を表示し続けます。");
							}
						}
				} else {
					if(SpeedMeter.manager.discreteMeterOnOff.containsKey(player.getName())) {
						if((!SpeedMeter.manager.discreteMeterOnOff.get(player.getName()))) {
							if(allMeterOnOffList.containsKey(player.getName()) || !allMeterOnOffList.containsKey(player.getName())) {
								if(!allMeterOnOffList.get(player.getName()) || allMeterOnOffList.get(player.getName()) == null) {
									sender.sendMessage(ChatColor.GRAY + "" + ChatColor.BOLD + "info: 速度は非表示です。");
									return true;
								}
									...
									sender.sendMessage(ChatColor.GRAY + "" + ChatColor.BOLD + "info: 速度は非表示です。");
									sender.sendMessage(ChatColor.GOLD + "" + ChatColor.BOLD +
											"Tip: " + ChatColor.GRAY + "" + ChatColor.BOLD + "もう一度 /meter on で常に速度が表示されます。");
									return true;
								}
							}
						}
			...
			if (args.length == 0) {
				Player player = (Player)sender;
				if(!this.allMeterOnOffList.get(player.getName()) || !this.allMeterOnOffList.containsKey(player.getName())) {
    ...
    public void initialize() {
    	...
    	AllowedWorldNames = config.getStringList("Allowed_World_Names");
   		...
   		PlayerMoveListener.playerSpeedThresholdValue =
   				config.getDouble("Player_Speed_Threshold", PlayerMoveListener.playerSpeedThresholdValue);
   		PlayerMoveListener.otherVehicleSpeedThresholdValue =
   				config.getDouble("Other_Vehicle_Speed_Threshold", PlayerMoveListener.otherVehicleSpeedThresholdValue);
   		PlayerMoveListener.vehicleSpeedThresholdValue =
   				config.getDouble("Vehicle_Speed_Threshold", PlayerMoveListener.vehicleSpeedThresholdValue);
    	manager = new Manager();
